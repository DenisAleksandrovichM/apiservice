...
func TestUserCreate(t *testing.T) {
	...
	t.Run("error", func(t *testing.T) {
		t.Run("validation error", func(t *testing.T) {
			// arrange
			f := userSetUp(t)
			f.userRepo.EXPECT().Create(f.ctx, user1).Return(modelsPkg.User{}, validate.ErrValidation)
			req := &api.UserCreateRequest{
				Login:     user1.Login,
				FirstName: user1.FirstName,
				LastName:  user1.LastName,
			...
			// act
			_, err := f.service.UserCreate(f.ctx, req)
			// assert
			assert.EqualError(t, err, status.Error(codes.InvalidArgument, validate.ErrValidation.Error()).Error())
		})
		t.Run("internal error", func(t *testing.T) {
			// arrange
			f := userSetUp(t)
			f.userRepo.EXPECT().Create(f.ctx, user1).Return(modelsPkg.User{}, errSome)
			// act
			req := &api.UserCreateRequest{
				Login:     user1.Login,
				FirstName: user1.FirstName,
...
func TestUserRead(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		req := &api.UserReadRequest{
			Login: user1.Login,
		}
		// act
		resp, err := f.service.UserRead(f.ctx, req)
		// assert
		require.NoError(t, err)
	...
	t.Run("error", func(t *testing.T) {
		t.Run("validation error", func(t *testing.T) {
			// arrange
			f := userSetUp(t)
			f.userRepo.EXPECT().Read(f.ctx, user1.Login).Return(modelsPkg.User{}, validate.ErrValidation)
			req := &api.UserReadRequest{
				Login: user1.Login,
			}
			// act
			_, err := f.service.UserRead(f.ctx, req)
			// assert
			assert.Equal(t, err.Error(), status.Error(codes.InvalidArgument, validate.ErrValidation.Error()).Error())
		})
		t.Run("internal error", func(t *testing.T) {
			// arrange
			f := userSetUp(t)
...
func TestUserUpdate(t *testing.T) {
	...
	t.Run("error", func(t *testing.T) {
		t.Run("validation error", func(t *testing.T) {
			// arrange
			f := userSetUp(t)
			f.userRepo.EXPECT().Update(f.ctx, user1).Return(modelsPkg.User{}, validate.ErrValidation)
			req := &api.UserUpdateRequest{
				Login:     user1.Login,
				FirstName: user1.FirstName,
				LastName:  user1.LastName,
		...
		t.Run("internal error", func(t *testing.T) {
			// arrange
			f := userSetUp(t)
			f.userRepo.EXPECT().Update(f.ctx, user1).Return(modelsPkg.User{}, errSome)
			req := &api.UserUpdateRequest{
				Login:     user1.Login,
				FirstName: user1.FirstName,
				LastName:  user1.LastName,
...
func TestUserDelete(t *testing.T) {
	...
	t.Run("error", func(t *testing.T) {
		t.Run("validation error", func(t *testing.T) {
			// arrange
			f := userSetUp(t)
			f.userRepo.EXPECT().Delete(f.ctx, user1.Login).Return(modelsPkg.User{}, validate.ErrValidation)
			req := &api.UserDeleteRequest{
				Login: user1.Login,
			}
			// act
			_, err := f.service.UserDelete(f.ctx, req)
			// assert
			assert.Equal(t, err.Error(), status.Error(codes.InvalidArgument, validate.ErrValidation.Error()).Error())
		})
		t.Run("internal error", func(t *testing.T) {
			// arrange
			f := userSetUp(t)
			f.userRepo.EXPECT().Delete(f.ctx, user1.Login).Return(modelsPkg.User{}, errSome)
			req := &api.UserDeleteRequest{
				Login: user1.Login,
			}
			// act
...
func TestUserList(t *testing.T) {
	...
	t.Run("error", func(t *testing.T) {
		...
		// act
		_, err := f.service.UserList(f.ctx, &api.UserListRequest{})
		// assert
		assert.Equal(t, err.Error(), status.Error(codes.Internal, errSome.Error()).Error())
	})
}
